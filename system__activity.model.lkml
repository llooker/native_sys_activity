...
datagroup: system_activity_dashboards_datagroup {
  max_cache_age: "12 hours"
}

persist_with: system__activity_default_datagroup
...
explore: content_usage {
  ...
  #_dashboard_linked_looks is defined in content_usage.view.lkml
  join: _dashboard_linked_looks {
    relationship: one_to_one
    sql_on: ${look.id} = ${_dashboard_linked_looks._id} ;;
  }
}
...
explore: dashboard {
  ...
  join: folders {
    type: left_outer
    sql_on: ${dashboard.space_id} = ${folders.id} ;;
    relationship: many_to_one
  }
  join: user {
    type: left_outer
...
explore: group {
  ...
  join: group_role {
    from: role
    type: left_outer
    sql_on: ${role_group.role_id} = ${group_role.id} ;;
    relationship: many_to_many
  }
}
...
explore: look {
  ...
  join: folders {
    type: left_outer
    sql_on: ${look.space_id} = ${folders.id} ;;
    relationship: many_to_one
  }
  join: user {
    type: left_outer
  ...
  join: history {
    fields: [history.id, history.status, history.message, history.source, history.issuer_source]
    sql_on: ${query.id} = ${history.query_id};;
    relationship: one_to_many
  }
}
...
# Commenting out Re: https://looker.atlassian.net/browse/ADMIN-1658
...
explore: scheduled_plan {
  ...
  join: scheduled_job_frequency {
    type: left_outer
    sql_on: ${scheduled_job.scheduled_plan_id} = ${scheduled_job_frequency.scheduled_plan_id} AND
            ${scheduled_job.id} = ${scheduled_job_frequency.scheduled_job_id} ;;
    relationship: one_to_one
  }
  join: scheduled_job_rank {
    type: left_outer
    sql_on: ${scheduled_plan.content_type_id} = ${scheduled_job_rank.content_type_id} ;;
    relationship: many_to_many
  }
...
explore: folders {
  join: user {
    type: left_outer
    sql_on: ${folders.creator_id} = ${user.id} ;;
    relationship: many_to_one
  }
  join: user_facts {
    type: left_outer
    sql_on: ${folders.creator_id} = ${user_facts.user_id} ;;
    relationship: many_to_one
  }
  join: user_facts_role {
    type: left_outer
...
explore: user {
  ...
  #Additional join to get names of sudoers. View _usernames located in users.view.lkml
  join: _usernames {
    view_label: "Session"
    type: left_outer
    sql_on: ${session.sudo_user_id} = ${_usernames.id} ;;
    relationship: many_to_one
    fields: [_usernames.sudo_user_name]
  }
  join: access_token {
    type: left_outer
  ...
  join: folders_user {
    type: left_outer
    sql_on: ${user.id} = ${folders_user.user_id} ;;
    relationship: one_to_many
    fields: []
  }
  join: folders {
    type: left_outer
    sql_on: ${folders_user.space_id} = ${folders.id} ;;
    relationship: many_to_many
  }
  join: scheduled_plan {
    type: left_outer
